...
def hough_transform(image):
    ...
    line_segments = cv2.HoughLinesP(image, rho, angle, min_threshold, np.array([]), minLineLength=10,
                                    maxLineGap=5)
...
def find_coordinates(image, line_params):
...
def test_image(image):
    ...
    image = cv2.imread(image)            #insert image file name
    combo_image = lane_follower.follow_lane(image)
    show_image("result", combo_image, True)
    cv2.waitKey(0)
